...
def del_stopwords(seg_sent):
    stopwords = read_lines(path+"/stop_word/sentiment_stopword.txt")  # 读取停用词表
    new_sent = []   # 去除停用词后的句子
...
def read_quanzhi(request):
    result_dict = []
    if request == "one":
        result_dict = read_lines(path+"/adverbs_of_degree_dictionary/most.txt")
    elif request == "two":
        result_dict = read_lines(path+"/adverbs_of_degree_dictionary//very.txt")
    elif request == "three":
        result_dict = read_lines(path+"/adverbs_of_degree_dictionary/more.txt")
    elif request == "four":
        result_dict = read_lines(path+"/adverbs_of_degree_dictionary/ish.txt")
    elif request == "five":
        result_dict = read_lines(path+"/adverbs_of_degree_dictionary/insufficiently.txt")
    elif request == "six":
        result_dict = read_lines(path+"/adverbs_of_degree_dictionary/inverse.txt")
    else:
        pass
    return result_dict
